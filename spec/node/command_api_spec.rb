...
describe 'Node Command API' do
  ...
  def test_command command, params = nil, response = nil, &block
    $responses = {}
    EM.run do
      @client = Bitcoin::Node::CommandClient.connect(*@config[:command]) do
        on_connected do
  ...
  before do
    Bitcoin::Blockchain::Validation::Block::RULES.merge({
    ...
    silence_output { @node = Bitcoin::Node::Node.new(@config) }
    @pid = fork do
      SimpleCov.running = false if defined?(SimpleCov)
      silence_output { @node.run }
    end
  ...
  it "should query info" do
    ...
    info["storage"].should == "archive::sqlite:/"
