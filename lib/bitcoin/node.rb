...
module Bitcoin::Node
  ...
  autoload :ConnectionHandler, 'bitcoin/node/connection_handler'
  autoload :CommandHandler, 'bitcoin/node/command_handler'
  autoload :CommandClient, 'bitcoin/node/command_client'
  ...
  class Node
    ...
    DEFAULT_CONFIG = {
      ...
      announce: false,
      external_port: nil,
      mode: :full, # TODO: is this still used anywhere?
      cache_head: true,
      index_nhash: false,
    ...
    def set_store
      ...
      @store = Bitcoin::Blockchain.create_store(backend, {
        db: config,
        mode: @config[:mode],
    ...
    def work_queue
      ...
      if @store.in_sync?
        if @store.is_a?(Bitcoin::Blockchain::Backends::Utxo) && @store.config[:utxo_cache] > 0
      ...
      while obj = @queue.shift
        ...
        rescue Bitcoin::Blockchain::Validation::ValidationError
